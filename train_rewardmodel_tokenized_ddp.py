...
from model.rewardmodel_tokenized import RewardModel
from config.rewardmodel_config import RewardModel_Config
from data.rewardmodel_dataset_tokenized import RW_Dataset
...
def train_rewardmodel(config):
    ...
    rewardmodel = RewardModel(config)
    ...
    while global_step < config.global_step:
        ...
        for batch in tqdm(train_dataloader):
            ...
            if global_step % restore_step == 1 & local_rank == 0:  # 只在rank=0的卡上进行weight restore
                # save model
                print(f"model state dict: {rewardmodel.state_dict().keys()}")
            ...
            if global_step % evaluate_step == 1:
                # evaluate
                evaluate_rewardmodel(config, model, rewardmodel, tokenizer, global_step)
